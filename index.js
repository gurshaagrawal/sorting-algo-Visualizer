        ...
        for(let j=0;j<array.length-i-1;j++)
        {
            if(array[j]>array[j+1])
            {
                for(let k=0;k<bars.length;k++)
                {
                    if(k!==j && k!==j+1)
                    {
                        bars[k].style.backgroundColor="deeppink";
                    }
                }
                let temp=array[j];
                array[j]=array[j+1];
                ...
                bars[j].style.backgroundColor="lightseagreen";
                bars[j+1].style.height=array[j+1]*heightFactor+ "px" ;
                bars[j+1].style.backgroundColor="lightseagreen";
...
async function swap(items, leftIndex, rightIndex, bars) {
    ...
    bars[leftIndex].style.backgroundColor = "lightseagreen";
    bars[rightIndex].style.height = items[rightIndex] * heightFactor + "px";
    bars[rightIndex].style.backgroundColor = "lightseagreen";
    await sleep(speedFactor);
  }
  ...
  async function partition(items, left, right) {
    ...
    bars[pivotIndex].style.backgroundColor = "chartreuse";
...
async function InsertionSort(array) {
    ...
    for (let i = 1; i < array.length; i++) {
      ...
      while (j >= 0 && array[j] > key) {
        array[j + 1] = array[j];
        bars[j + 1].style.height = array[j + 1] * heightFactor + "px";
        bars[j + 1].style.backgroundColor = "chartreuse";
        await sleep(speedFactor);
        for (let k = 0; k < bars.length; k++) {
          if (k != j + 1) {
            bars[k].style.backgroundColor = "deeppink";
  ...
  async function swap(array, i, j, bars) {
    ...
    bars[j].style.backgroundColor = "chartreuse";
    await sleep(speedFactor);
